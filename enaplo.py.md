##  E-Napló Database and Statistics System  
    
### Table of Contents 
* [Database Initialization](#database-initialization)
* [Sample Data Insertion](#sample-data-insertion) 
* [Grade Calculations](#grade-calculations)
...
This section defines the database connection and creates the tables for the E-Napló system. 
...
This section inserts sample data into the database for testing and demonstration purposes.
...
### Grade Calculations 
This section defines functions for calculating weighted average grades and displaying results. 

```python
# Function to calculate weighted average grades
def felevijegy():
    c.execute("BEGIN IMMEDIATE TRANSACTION")
    veznev=input("Kérlek add meg a kért tanuló vezetéknevét: ")
    kernev=input("Kérlek add meg a kért tanuló keresztnevét: ")
    anyjaneve=input("Kérlek add meg a kért tanuló anyjának nevét: ")
    c.execute(''' 
    ...
    for jegy in jegyek_lista:
        ...
        if jegyek_lista:
            ...
            print("A tanulónak nincsenek érdemjegyei")
    connect.commit()

def tanuloatlag():
    c.execute("BEGIN IMMEDIATE TRANSACTION")
    veznev=input("Kérlek add meg a hozzáadni kért tanuló vezetéknevét: ")
    kernev=input("Kérlek add meg a hozzáadni kért tanuló keresztnevét: ")
    anyjaneve=input("Kérlek add meg a hozzáadni kért anyjának nevét: ")
    for i in range(1,11):
        c.execute(''' 
        SELECT jegy, veznev, kernev, jegy_tipus
        FROM tanulok
    ...
    print(jegy[1] + " " + jegy[2] +" félévi jegy : " + str(atlag))
    connect.commit()
```

### Menu System 
This section implements the interactive menu system for the E-Napló application.
...
### Student Management 
This section defines functions for adding, editing, and deleting students.
...
def tanhozz():
    ...
    kernev=input("Kérlek add meg a hozzáadni kért tanuló keresztnevét: ")
    anyjaneve=input("Kérlek add meg a hozzáadni kért anyjának nevét: ")
    kor=input("Kérlek add meg a hozzáadni kért tanuló életkorát: ")
    osztaly=input("Kérlek add meg a hozzáadni kért tanuló osztályát (A-D): ")
...
def tanszerk():
    c.execute("BEGIN IMMEDIATE TRANSACTION")
    veznev=input("Kérlek add meg a szerkeszteni kért tanuló vezetéknevét: ")
    kernev=input("Kérlek add meg a szerkeszteni kért tanuló keresztnevét: ")
    anyjaneve=input("Kérlek add meg a szerkeszteni kért anyjának nevét: ")
    ...
    if val=='O':
        uj_osztaly=input("Melyik osztályba helyezzük át a kiválasztott tanulót?: ")
        c.execute('''
        UPDATE tanulok
        SET osztaly=?
        WHERE tanulok.veznev=? AND tanulok.kevnev=? AND tanulok.anyjaneve=?
...
def tantorl():
    c.execute("BEGIN IMMEDIATE TRANSACTION")
    veznev=input("Kérlek add meg a törölni kért tanuló vezetéknevét: ")
    kernev=input("Kérlek add meg a törölni kért tanuló keresztnevét: ")
    anyjaneve=input("Kérlek add meg a törölni kért anyjának nevét: ")
    kor=0
    while True:
        c.execute(''' 
        SELECT kor
...
### Grade Management 
This section defines functions for adding, editing, and deleting grades.

```python
def menu_tanulok():
    while True:
        print("1 - Félévi jegy lekérdezése a tanulóra adott tárgyból")
    ...
    while True:
        try:
            ...
            if jegy in erdjegyek:
                ...
                print("Nem érdemjegyet írtál be, kérlek 1 és 5 között értékeljed a tanulót")
        except ValueError:
            print("Csak számokat adj meg! Próbáld újra.")
...
def jegy_szerk():
    c.execute("BEGIN IMMEDIATE TRANSACTION")
    tanvezn=input("Add meg kérlek az érdemjegy tulajdonosának vezetéknevét: ")
    tankern=input("Add meg kérlek az érdemjegy tulajdonosának keresztnevét: ")
    anyjanev=input("Add meg kérlek az érdemjegy tulajdonosa édesanyjának nevét: ")
    ...
    if check!=False:
        ...
        print("Az adott feltételekkel érdemjegy nem található a naplóban")
    connect.commit()
...
def jegy_torl():
    ...
    check=c.fetchall
    if check!=False:
        j=input("A rendszer talált egy jegyet ("+ check[0] + "), biztosan törölni szeretnéd? ( I - igen , N - nem, vissza az előző menübe)")
        if j=='I':
            c.execute(''' 
            DELETE
            FROM jegyek
...
### Subject Management 
This section defines functions for managing subject statistics.
